...
const Skills = ({ name, x, y }: Props) => {
    ...
    return (
        <motion.div className='flex items-center justify-center rounded-full font-semibold bg-dark text-ligth py-3 px-6 shadow-dark cursor-pointer absolute dark:text-dark dark:bg-ligth' whileHover={{ scale: 1.05 }} initial={{ x: 0, y: 0 }} whileInView={{ x: x, y: y, transition: { duration: 1.5 } }}>
            {name}
        </motion.div>
    )
}
